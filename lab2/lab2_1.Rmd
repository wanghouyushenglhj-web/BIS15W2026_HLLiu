...
Grey box called Code chunk
```{r}
getwd()
```

making new code chunk

1.cope from previous
...
```{r}
x <- 42 #this means x gets the value of 42
```
...
```{r}
mean(c(2,8,2)) #c stands for combine and creat a vector
```
...
```{r}
my_integer <- as.integer(my_numeric)#important with when we work with plot
```
